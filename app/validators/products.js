...
// Creamos una propiedad por cada formulario que se vaya a validar
module.exports = {
    create: [
        check("name").trim()
        ...
        check("brand").trim()
            .notEmpty().withMessage("Campo obligatorio").bail()
            .isLength({ min:4, max:100 }).withMessage("La marca debe tener entre 4 y 100 caracteres").bail(),
        ...
        check("price").trim()
            .notEmpty().withMessage("Campo obligatorio").bail()
            .isFloat({ min:0, max:9999999.99 }).withMessage("El precio no puede ser negativo").bail(),
    ],
